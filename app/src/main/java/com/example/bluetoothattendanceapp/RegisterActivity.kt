...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun validateTeacherInputs(): Boolean {
        ...
        val surname = findViewById<EditText>(R.id.surnameInput).text.toString().trim()
    ...
    private fun registerTeacher() {
        if (!validateTeacherInputs()) return
        ...
        lifecycleScope.launch {
            try {
                val result = firebaseManager.registerUser(
                    ...
                    password = findViewById<EditText>(R.id.passwordInput).text.toString().trim(),
                    username = findViewById<EditText>(R.id.emailInput).text.toString().trim(), // Email'i username olarak kullan
                    name = findViewById<EditText>(R.id.nameInput).text.toString().trim(),
                    surname = findViewById<EditText>(R.id.surnameInput).text.toString().trim(),
                ...
                result.onSuccess { user ->
                    ...
                    Toast.makeText(this@RegisterActivity, "Kayıt başarısız: ${exception.message}", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                hideLoading()
                Toast.makeText(this@RegisterActivity, "Bir hata oluştu: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
